...
export default class BackgroundCommand implements Command {
  ...
  async run({ originalMessage, args }: CommandContext): Promise<void> {
    ...
    if (args.length === 0 && !originalMessage.attachments.first()) {
      const { background_image } = await Users.findOne({
    ...
    if (args.length === 0) {
      ...
      if (user_attachment) {
        ...
        author.background_image = user_attachment.url

        await Users.save(author)

        const attachment = new MessageAttachment(user_attachment.url)
    ...
    author.background_image = args[0]
    author.balance -= amount

    await Users.save(author)

    const attachment = new MessageAttachment(author.background_image)
