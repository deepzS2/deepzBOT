...
import { User } from '@bot/database/entities/User'
...
export default class ProfileCommand implements Command {
  ...
  async run({ args, originalMessage }: CommandContext): Promise<void> {
    ...
    if (!args[0]) {
      background = author.background_image
      ...
      const user = await Users.findOneOrFail({ where: { id: mention.user.id } })
...
async function loadProfile(author: User, message: Message, avatar, bg = null) {
  await message.channel.startTyping()

  const { bio, reputation, balance, xp } = author
