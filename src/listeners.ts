...
export default class Listeners {
  ...
  private async checkIfUserExists(id: string, username: string) {
    ...
    try {
      const user = await Users.findOneOrFail({ where: { id: id } })

      if (!user) {
        await insertUser(id, username)
  ...
  private async onMessage(message: Message) {
    ...
    if (user) {
      user.xp += Math.floor(Math.random() * 15) + 10
      user.balance += Math.floor(Math.random() * 7) + 3
  ...
  private async onGuildDelete(guild: Guild): Promise<void> {
    ...
    await Guilds.delete({ id: guild.id })
  }
  ...
  private async reactions(
    ...
    try {
      const Guilds = await GuildsRepository()
      const guild = await Guilds.findOneOrFail({
      ...
      const { role_message: roleMessage, roles } = guild
